...
<div class="bg-white">
    <nav aria-label="Breadcrumb" class="flex mb-5">
        <ol class="flex overflow-hidden rounded-lg border border-gray-200 text-gray-600">
            ...
            <li class="relative flex items-center">
                ...
                <div
                    class="flex h-10 items-center bg-white pe-4 ps-8 text-xs font-medium transition hover:text-gray-900 {{ Request::routeIs(['stocks.edit']) ? 'bg-gray-100 dark:bg-orange-700 text-gray-600 dark:text-white dark:hover:text-white ' : '' }} text-gray-600 px-4 transition hover:text-gray-900">
                    Edit Stok
                </div>
            </li>
        </ol>
    ...
    <form action="{{ route('stocks.update', $stock->id) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <select name="menu_id" id="menu_id" class="w-full px-4 py-2 border rounded-lg border-gray-300">
                @foreach($menus as $menu)
                <option value="{{ $menu->id }}" {{ $stock->menu_id == $menu->id ? 'selected' : '' }}>{{ $menu->nama_menu
                    }}</option>
                @endforeach
            </select>
        </div>
